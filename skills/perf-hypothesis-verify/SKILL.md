...
### 12. 스크립트 인라인 실행 → 재현 불가, 레포에 증거 없음 ★ (신규)
증상: 실험이 끝났는데 실행한 명령어가 대화창에만 남아 있음.
     보고서에 측정값이 있지만 "어떻게 측정했는가"의 재현 경로가 없음.
     나중에 "이 값 어떻게 나온 거야?"를 설명하기 어려움.
진단:
  - 실험 디렉토리에 스크립트 파일이 없고 보고서 파일만 있음
  - mongosh --eval '...', redis-cli 직접 입력 등 인라인으로만 실행
  - 가이드(실험 설계서)도 파일로 저장 안 됨
대응:
  1. 모든 실험 스크립트를 파일로 먼저 작성한 뒤 실행
     (mongosh scripts/01_setup.js, go run bench_control.go 등)
  2. 실험 설계 가이드도 experiment-guide.md로 파일 저장
  3. Step 3 디렉토리 구조에 맞춰 실험 전 파일 골격 먼저 생성
실측 사례: MongoDB Partial Index 실험 — 스크립트를 인라인으로만 실행 →
  보고서 commit 후 사용자가 "작성하거나 사용한 파일은 없다는 거야?" 지적 →
  스크립트 7개 + 가이드 문서를 사후에 복원해 별도 commit 필요
